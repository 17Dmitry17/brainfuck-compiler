...
_LEAF_MAP = {'+': IncrNode, '-': DecrNode, '>': MoveRNode, '<': MoveLNode, '.': OutputNode, ',': InputNode}
    ...
    for cmd, pos in iterator:
        if cmd in _LEAF_MAP:
            ...
            if opening_pos is None:
                raise SyntaxError(f"Unexpected ']' at position {pos}: no matching '[' found.")
            return children
        else:
            raise SyntaxError(f'Unknown token {cmd!r} at position {pos}.')
    if opening_pos is not None:
        raise SyntaxError(f"Unclosed '[' at position {opening_pos}: end of input reached without a matching ']'.")
    return children