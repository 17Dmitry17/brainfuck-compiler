...
def _walk(nodes: list, depth: int, errors: list[str]) -> None:
    for node in nodes:
        if isinstance(node, LoopNode):
            if depth + 1 > 100:
                errors.append(f'Ошибка: слишком глубокая вложенность циклов (больше 100 уровней) на позиции {node.pos}')
            if len(node.children) == 0:
                errors.append(f'Предупреждение: пустой цикл [] на позиции {node.pos}')
