...
_OPPOSITES = {IncrNode: DecrNode, DecrNode: IncrNode, MoveRNode: MoveLNode, MoveLNode: MoveRNode}
...
def _merge_runs(nodes: list) -> list:
    ...
    current = nodes[0]
    for node in nodes[1:]:
        if isinstance(node, _COUNTABLE) and type(node) is type(current):
...
def _cancel_opposites(nodes: list) -> list:
    ...
    while changed:
        ...
        while i < len(nodes):
            node = nodes[i]
            if i + 1 < len(nodes) and isinstance(node, _COUNTABLE) and (type(nodes[i + 1]) is _OPPOSITES.get(type(node))):
                a, b = (node, nodes[i + 1])
